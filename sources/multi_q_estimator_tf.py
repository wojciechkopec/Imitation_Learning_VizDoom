...
class QEstimator:
    def __init__(self, available_actions_count, resolution, subnets=5, create_convolution_layers=None, replay_memory_size=10000,
    ...
    def _create_network(self, available_actions_count, resolution):
        ...
        inputs_shape = conv2.get_shape()

        dims = inputs_shape[1:]
        k = dims.num_elements()
        conv2_flat = tf.reshape(conv2, [-1, k, 1])

        first_dense_nodes = 128
        fc1 = tf.contrib.layers.convolution2d(conv2_flat, num_outputs=first_dense_nodes * self.subnets,
                                              kernel_size=[first_dense_nodes], stride=[first_dense_nodes],
                                              activation_fn=tf.nn.relu,
                                              weights_initializer=tf.contrib.layers.xavier_initializer(),
                                              biases_initializer=tf.constant_initializer(0.1))

        q = tf.contrib.layers.convolution2d(fc1, num_outputs=available_actions_count * self.subnets,
                                            kernel_size=[available_actions_count], stride=[available_actions_count],
                                            activation_fn=None,
                                            weights_initializer=tf.contrib.layers.xavier_initializer(),
                                            biases_initializer=tf.constant_initializer(0.1))
        qs = tf.reshape(q, [-1, self.subnets, available_actions_count])
    ...
    def learn_from_transition(self, s1, a, s2, s2_isterminal, r, q2=float("-inf")):
        ...
        if self.memory.size > self.batch_size:
            ...
            q2 = np.max(self.get_q_values(s2), axis=2)
            ...
            for ind in range(self.batch_size):
                for net in mask[ind]:
                    target_q[ind, net, a[ind]] = r[ind] + self.discount_factor * (1 - isterminal[ind]) * q2[ind,net]
